...
public:
    ...
    long clk = 0;
    DRAM<T>* channel;
    BlockHammer bh;            // Object to impliment BlockHammer
    ...
    {
        clk++;
        bh.eUpdate(clk);                // Clock sync for memory controller and blockhammer
        ...
        if (cmd == T::Command::ACT){                        // We check only ACTs for Hammering, and then process them based on blockhammer
            if(!bh.BHcheck((*req).addr, clk,(*req).coreid, get_addr_vec(cmd, req)[int(T::Level::Bank)])){
